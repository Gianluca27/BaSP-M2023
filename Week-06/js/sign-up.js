...
const errorMessages = {
  name: 'Name is required and can only contain alphabetic characters',
  lastName: 'Last name is required and can only contain alphabetic characters',
  dni: 'DNI is required and can only contain numeric characters',
  bdayDate: 'Please enter a valid date of birth',
  telNumber: 'Please enter a valid phone number',
  address: 'Address is required',
  locality: 'Locality is required',
  postalCode: 'Postal code is required and can only contain numeric characters',
...
function validateInput(input, errorMessage) {
  ...
  if (errorMessage === errorMessages.name || errorMessage === errorMessages.lastName) {
    if (!/^[a-zA-Z ]+$/.test(input.value)) {
      showError(input, errorMessage);
      return false;
    }
  }
  if (errorMessage === errorMessages.dni || errorMessage === errorMessages.postalCode) {
    if (!/^[0-9]+$/.test(input.value)) {
      showError(input, errorMessage);
      return false;
    }
  }
  if (errorMessage === errorMessages.bdayDate) {
    if (!isValidDate(input.value)) {
      showError(input, errorMessage);
      return false;
    }
  }
  if (errorMessage === errorMessages.telNumber) {
    if (!/^\d{10}$/.test(input.value)) {
      showError(input, errorMessage);
      return false;
    }
  }
  if (errorMessage === errorMessages.email) {
    if (!/\S+@\S+\.\S+/.test(input.value)) {
      showError(input, errorMessage);
      return false;
    }
  }
  if (errorMessage === errorMessages.password) {
    if (input.value.length < 8) {
      showError(input, errorMessage);
      return false;
    }
  }
  if (errorMessage === errorMessages.repeatPass) {
